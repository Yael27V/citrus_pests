import { Link } from 'react-router-dom';

export default function PlagaCard({ plaga }) {
  const imagen = plaga.imagen_cover_url || plaga.galerias?.adultos?.[0] || plaga.galerias?.huevos?.[0] || null;
  
  return (
    <Link 
      to={`/plagas/${plaga.id}`}
      className="group block bg-white border border-stone-200 overflow-hidden hover:border-stone-300 transition-colors"
    >
      <div className="relative h-44 overflow-hidden bg-stone-100">
        {imagen ? (
          <img 
            src={imagen} 
            alt={plaga.nombre_comun}
            className="w-full h-full object-cover"
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center">
            <svg className="w-10 h-10 text-stone-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
            </svg>
          </div>
        )}
        <div className="absolute top-3 right-3">
          <span className="inline-flex items-center px-2 py-1 bg-stone-900 text-white text-xs font-medium">
            {plaga.clasificacion?.split(' - ')[0] || 'Plaga'}
          </span>
        </div>
      </div>
      <div className="p-5">
        <h3 className="font-semibold text-lg text-stone-900 mb-1">
          {plaga.nombre_comun}
        </h3>
        <p className="text-sm text-stone-500 italic mb-3">
          {plaga.nombre_cientifico || 'Sin nombre científico'}
        </p>
        <p className="text-stone-600 text-sm leading-relaxed line-clamp-2">
          {plaga.descripcion_corta || 'Sin descripción disponible'}
        </p>
        <div className="mt-4 flex items-center text-green-700 font-medium text-sm">
          Ver detalles
          <svg className="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
          </svg>
        </div>
      </div>
    </Link>
  );
}
